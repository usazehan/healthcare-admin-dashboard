...
import com.healthcare.appointment.grpc.toProto
...
class AppointmentGrpcService(
     ...
     */
    override fun createAppointment(request: CreateAppointmentRequest, responseObserver: StreamObserver<ProtoAppointment>) {
        try {
    ...
    override fun getAppointment(request: GetAppointmentRequest, responseObserver: StreamObserver<ProtoAppointment>) {
        try {
    ...
    override fun listAppointments(
        ...
        try {
            ...
            val response = ListAppointmentsResponse.newBuilder()
    ...
    override fun updateAppointment(request: UpdateAppointmentRequest, responseObserver: StreamObserver<ProtoAppointment>) {
        try {
            ...
            val updatedAppointment =
                    existingAppointment.copy(
                            startTime = LocalDateTime.parse(request.startTime, DateTimeFormatter.ISO_DATE_TIME),
                            endTime = LocalDateTime.parse(request.endTime, DateTimeFormatter.ISO_DATE_TIME),
                            type = request.type.toDomain(),
                            notes = request.notes,
                            reason = request.reason,
                            status = AppointmentStatus.SCHEDULED,
    ...
    override fun cancelAppointment(request: CancelAppointmentRequest, responseObserver: StreamObserver<ProtoAppointment>) {
        try {
            val appointment = appointmentService.getAppointment(request.id.toLong())
            ...
            val updatedAppointment = appointmentService.updateAppointmentStatus(
                id = appointment.id,
    ...
    override fun changeAppointmentStatus(request: ChangeAppointmentStatusRequest, responseObserver: StreamObserver<ProtoAppointment>) {
        try {
            val appointment = appointmentService.getAppointment(request.id.toLong())
